...
export function SiteHeader() {
  ...
  /* Passage en desktop : le panneau mobile n'a plus lieu d'être. */
  useEffect(() => {
  ...
  return (
    <header className="sticky top-0 z-50">
      ...
      <div className="relative mx-auto flex h-16 w-full max-w-(--container-page) items-center justify-between gap-3 px-4 sm:px-6 md:h-18">
        <Link
          href="/"
        ...
        >
          {site.nav.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="rounded-pill px-3.5 py-2 text-[0.9rem] text-ink-2 transition-colors duration-200 ease-(--ease-out-soft) hover:bg-surface-muted hover:text-ink"
            >
              {item.label}
            </Link>
          ))}
        ...
        <div className="relative z-10 flex shrink-0 items-center gap-1">
          ...
          <Link
            ...
            className="hidden rounded-pill px-3.5 py-2 text-[0.9rem] text-ink-2 transition-colors duration-200 ease-(--ease-out-soft) hover:bg-surface-muted hover:text-ink md:inline-flex"
          >
            {site.cta.connexion}
          </Link>
          ...
          <button
            ...
            className="inline-flex size-10 items-center justify-center rounded-pill text-ink-2 transition-colors duration-200 hover:bg-surface-muted hover:text-ink lg:hidden"
          >
            {open ? (
      ...
      <AnimatePresence>
        ...
        {open ? (
          <motion.div
            ...
            className="fixed inset-x-0 bottom-0 z-50 rounded-t-sheet bg-surface px-4 pb-8 pt-3 shadow-float ring-1 ring-line lg:hidden"
          >
